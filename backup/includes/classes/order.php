...
/* END: ADD MY CUSTOM DETAILS */
      ...
      $this->products_ordered_html .=
      '<tr>' . "\n" .
      '<td class="product-details" align="left" valign="top" width="30" style="color:#666666;">' . $this->products[$i]['qty'] . '&nbsp;x</td>' . "\n" .
      '<td class="product-details" valign="top" style="color:#666666;">' . nl2br($this->products[$i]['name']) . ($this->products[$i]['model'] != '' ? ' (' . nl2br($this->products[$i]['model']) . ') ' : '') . "\n" .
      '<nobr><small style="color:#666666;"><em> '. nl2br($this->products_ordered_attributes) .'</em></small></nobr></td>' . "\n" .
      '<td class="product-details-num" valign="top" align="right" style="color:#666666;">' .
      $currencies->display_price($this->products[$i]['final_price'], $this->products[$i]['tax'], $this->products[$i]['qty']) .
      ($this->products[$i]['onetime_charges'] !=0 ?
      '</td></tr>' . "\n" . '<tr><td class="product-details" style="color:#666666;">' . nl2br(TEXT_ONETIME_CHARGES_EMAIL) . '</td>' . "\n" .
      '<td style="color:#666666;">' . $currencies->display_price($this->products[$i]['onetime_charges'], $this->products[$i]['tax'], 1) : '') .
      '</td></tr>' . "\n";
    }
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    //order totals area
    $html_ot .= '<td class="order-totals-text" align="right" width="100%" style="color:#666666;">' . '&nbsp;' . '</td> ' . "\n" . '<td class="order-totals-num" align="right" nowrap="nowrap">'.'</td> </tr>' . "\n" . '<tr>';
    for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
      $email_order .= strip_tags($order_totals[$i]['title']) . ' ' . strip_tags($order_totals[$i]['text']) . "\n";
      $html_ot .= '<td class="order-totals-text" align="right" width="100%" style="color:#666666;">' . $order_totals[$i]['title'] . '</td> ' . "\n" . '<td class="order-totals-num" align="right" nowrap="nowrap" style="color:#666666;">' .($order_totals[$i]['text']) .'</td> </tr>' . "\n" . '<tr>';
    }
