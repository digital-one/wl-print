...
<h1 id="ReviewDefaultHeading">Review Centre - <?php echo $products_name; ?></h1>
<div id="review_header">
	<p><?php echo sprintf(TEXT_REVIEW_DATE_ADDED, zen_date_short($review_info->fields['date_added'])); ?></p>
	<p><?php echo sprintf(TEXT_REVIEW_BY, zen_output_string_protected($review_info->fields['customers_name'])); ?></p>
</div>
<?php
  if (zen_not_null($products_image)) {
     ...
     */
?>
<div id="reviewsInfoDefaultProductImage" class="centeredContent back"><?php require($template->get_template_dir('/tpl_modules_main_review_image.php',DIR_WS_TEMPLATE, $current_page_base,'templates'). '/tpl_modules_main_review_image.php'); ?>
	
	<h3 class="rating"><?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $review_info->fields['reviews_rating'] . '.gif', sprintf(TEXT_OF_5_STARS, $review_info->fields['reviews_rating'])), sprintf(TEXT_OF_5_STARS, $review_info->fields['reviews_rating']); ?>
	<br /><?php echo $review_info->fields['reviews_rating']; ?> out 5 Bubbles&nbsp;&nbsp;&nbsp;
	</h3>
...
<div id="reviewsInfoDefaultReviewsListingLink" class="buttonRow"><?php echo '<a href="' . zen_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, zen_get_all_get_params(array('reviews_id'))) . '">' . zen_image_button(BUTTON_IMAGE_WRITE_REVIEW, BUTTON_WRITE_REVIEW_ALT) . '</a>'; ?></div>
</div>
