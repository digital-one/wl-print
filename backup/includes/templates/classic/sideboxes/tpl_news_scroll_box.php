...
$shown_news = 1; // Number of Shown News
...
';
$content .= '';
$content .= '<p class="link"><a href="' . zen_href_link('news_archiv', '', 'NONSSL') . '">Click here for more news...</a></p>';
$content .= '</div>';
	...
	if ($news_box_query->RecordCount() >= 1) {
		...
		$rows = 0;
			while (!$news_box_query->EOF) {
				...
				$news_list[$rows]['title'] = substr(strip_tags($news_box_query->fields['news_content']),0,$news_title_lenth);
				$news_list[$rows]['dateadded'] = zen_date_short($news_box_query->fields['news_added_date'],0,10);
				$news_list[$rows]['date'] = substr($news_box_query->fields['news_added_date'],0,10);
				$news_list[$rows]['content'] = substr(strip_tags($news_box_query->fields['news_content']),$news_title_lenth+2,60);
				$news_box_query->MoveNext();
			};
			for ($i=1; $i<=sizeof($news_list); $i++) {


				$content .= '<p class="date">' . $news_list[$i]['dateadded'] . '</p>';
				$content .= '<p><strong><a href="' . zen_href_link('more_news&news_id=' . $news_list[$i]['id'], '', 'NONSSL') . '">' . $news_list[$i]['title'] . '</a></strong></p>';
				$content .= '<p>' . $news_list[$i]['content'] . '...</p>';
