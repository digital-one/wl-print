...
if (MAX_DISPLAY_SPECIAL_PRODUCTS > 0 ) {
  ...
  if ($num_products_count) {
    ...
    while (!$specials->EOF) {
      ...
      $list_box_contents[$row][$col] = array('params' => 'class="specialsListBoxContents col' . $col . ' " ' . 'style="width:150px;"',
                                             'text' => '
<p class="name"><a href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($specials->fields['master_categories_id']) . '&products_id=' . $specials->fields['products_id']) . '">' . $specials->fields['products_name'] . '</a></p><a href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($specials->fields['master_categories_id']) . '&products_id=' . $specials->fields['products_id']) . '">' . (($specials->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : zen_image(DIR_WS_IMAGES . $specials->fields['products_image'], $specials->fields['products_name'], MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT) . '</a>') . '<p class="price">' . $products_price);
      $col ++;
