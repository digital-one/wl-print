...
//$product_image_new_full = $xsell_query->fields['products_image'];
...
//$product_image_file = strip_ext($product_image_new_full);
//$product_image_new = $product_image_file . '&#95;MED&#46;jpg';


  $list_box_contents[$row][$col] = array('params' => 'class="centerBoxContentsCrossSell' . $col . ' centeredContent back"' . ' ' . 'style="width:150px;"',
  'text' => '<p class="name"><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . $xsell_query->fields['products_id']) . '">' . $xsell_query->fields['products_name'] . '</a>' . (XSELL_DISPLAY_PRICE=='true'? '</p><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $xsell_query->fields['products_image'], $xsell_query->fields['products_name'], MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT) . '</a><p class="price">'.zen_get_products_display_price($xsell_query->fields['products_id']):'') . '</p>' );
  $col ++;
