<?php
/*
  $Id: stats_monthly_sales.php, v 1.3 2008/08/14  $
  ...
  Thanks to nosnappyname for PHP 5.0 fix.
  Thanks to barco57 for the dropdown status fix.
...
works with Zen Cart 1.3.8a it might work with other version but not tested.
...
if (isset($_GET['show'])) {
	...
	$sel_day = 0;
	if (isset($_GET['day'])) $sel_day = zen_db_prepare_input($_GET['day']);
	$status = '';
	if ($_GET['status']) $status = zen_db_prepare_input($_GET['status']);
...
<link rel="stylesheet" type="text/css" href="<?php if(!$print) {
	...
	else echo 'includes/stylesheet_print.css'; ?>"> 
-->
